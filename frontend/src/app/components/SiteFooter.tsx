...
export function SiteFooter() {
  return (
    <footer style={{ background: "#1A1620", padding: "40px 32px 24px", flexShrink: 0 }}>
          ...
          <div className="flex items-center justify-between" style={{ flexWrap: "wrap", gap: 10 }}>
            <div className="flex items-center" style={{ gap: 18 }}>
              {policyLinks.map((label) => (
                <span key={label} style={{ color: "#C4BDCC", fontSize: 11.5 }}>{label}</span>
              ))}
            </div>
            <span style={{ color: "#A39CAC", fontSize: 11 }}>© {new Date().getFullYear()} OneShop. All rights reserved.</span>
          </div>
        </div>
      </div>
    </footer>
